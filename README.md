...
1. **CGNAT (Carrier-Grade NAT) 帯域の仕様**:
   ...
   AdGuard Home はデフォルトで `100.x.x.x` をパブリック IP とみなすため、通常の「プライベートリバースDNSサーバー」一覧に `100.100.100.100` とだけ記述しても、クエリがパブリックアップストリーム（Cloudflare 等）へ飛んでしまい `NXDOMAIN` になります。
2. **ドメイン限定アップストリーム構文の使用**:
   以下のように `.in-addr.arpa` および `.ts.net` のドメイン指定構文を使用することで、Tailscale のクエリを確実に MagicDNS (`100.100.100.100`) へ転送できます。

### 設定手順 (AdGuard Home Web UI)

1. AdGuard Home の設定画面から **設定** ➔ **DNS設定** を開きます。
2. **アップストリームDNSサーバー** に以下を追加します：
   ```text
   [/100.in-addr.arpa/]100.100.100.100
   [/ts.net/]100.100.100.100
   ```
3. **プライベートリバースDNSサーバー** に以下を追加します：
   ```text
   [/100.in-addr.arpa/]100.100.100.100
   ```
